...
def test(args, model, tokenizer, test_dataset, best_threshold=0.5):
    ...
    df.to_csv("../data/raw_predictions/CodeT5/VulRepair_raw_preds_wild2_beam1.csv")
...
def main():
    ...
    if args.do_test:
        ...
        tokenizer.add_tokens(["<S2SV_StartBug>", "<S2SV_EndBug>", "<S2SV_blank>", "<S2SV_ModStart>", "<S2SV_ModEnd>"])
        model.resize_token_embeddings(len(tokenizer))
        model.load_state_dict(torch.load('./saved_models/checkpoint-best-loss/model_one_stmt_final.bin'))
        model.to(args.device)
        pairs=os.listdir('wild2_T5')
        dataset = {'name': [], 'source': [], 'target': []}
        for i, pair in enumerate(pairs):
            #if i>30:
            #    break
            ...
            #continue
            try:
                f=open('./wild2_T5/'+pair+'/'+pair+'_in.c')
                source=f.read()
                source=source.replace(' ', '<S2SV_blank>')
                source=source.replace('!@#$', ' ')
                f.close()
                f=open('./wild2_T5/'+pair+'/'+pair+'_out.c')
                target=f.read()
                target=target.replace(' ', '<S2SV_blank>')
                target=target.replace('!@#$', ' ')
                f.close()
