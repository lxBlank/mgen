...
def test(args, model, tokenizer, test_dataset, best_threshold=0.5):
    ...
    df.to_csv("../data/raw_predictions/CodeT5/VulRepair_raw_preds_translate_final_beam10.csv")
...
def main():
    ...
    if args.do_test:
        ...
        for i, pair in enumerate(pairs):
            ...
            #continue
            try:
                f=open('./T5_vulgen_test_translate_final_tokenized/'+pair+'/'+pair+'_in.c')
                source=f.read()
                source=source.replace(' ', '<S2SV_blank>')
                source=source.replace('!@#$', ' ')
                f.close()
                f=open('./T5_vulgen_test_translate_final_tokenized/'+pair+'/'+pair+'_out.c')
                target=f.read()
                target=target.replace(' ', '<S2SV_blank>')
                target=target.replace('!@#$', ' ')
                f.close()
